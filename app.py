...
# 4. Fundamental Filters (Relaxed to include more stocks)
st.subheader("🔍 Fundamental Filters")
col1, col2 = st.columns(2)
with col1:
    min_pe = st.number_input("Min P/E Ratio", value=5)  # Lowered from 10
    min_roe = st.number_input("Min ROE (%)", value=10)  # Lowered from 15
with col2:
    max_debt_equity = st.number_input("Max Debt/Equity", value=1.5)  # Increased from 1.0
    min_market_cap = st.number_input("Min Market Cap ($B)", value=0.5)  # Lowered from 1
...
min_rsi = st.slider("Min RSI (Avoid Overbought)", 30, 70, 40)  # Lowered from 50
min_volume = st.number_input("Min Avg Volume (Millions)", value=0.5)  # Lowered from 1M
...
def calculate_momentum(ticker, lookback_days):
    try:
        data = yf.download(ticker + ".NS", period=f"{lookback_days}d")["Close"]
        return (data[-1] / data[0] - 1) * 100  # Return %
    except Exception:
        return np.nan  # Return NaN if data is unavailable

if st.button("Run Momentum Scan"):
    with st.spinner(f"Scanning {selected_sheet} for top momentum stocks..."):
        momentum_data = []

        for ticker in tickers[:50]:  # Limit to 50 for faster execution (remove [:50] for full scan)
            try:
                # Get momentum
                ...
                rsi = 70 - (hist["Close"].pct_change().mean() * 100) if not hist.empty else np.nan
                avg_volume = hist["Volume"].mean() / 1e6 if not hist.empty else np.nan  # In millions
                ...
                if (
                    ...
                    and (np.isnan(rsi) or rsi >= min_rsi)  # Allow NaN RSI
                    and (np.isnan(avg_volume) or avg_volume >= min_volume)  # Allow NaN Volume
                ):
        ...
        allocation = {"Low": 5, "Medium": 8, "High": 12}[risk_tolerance]
    ...
    # Add disclaimer
    st.warning("**Disclaimer:** This is for educational purposes only. Past performance is not indicative of future results. Always do your own research before investing.")
